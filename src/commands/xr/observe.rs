...
use crate::commands::xr::helpers::manifest::{
    emit_report, ensure_mapping, load_specs, match_spec, render_manifest,
    sanitize_manifest_defaults, set_observe_only_management, strip_external_name_fields, vs,
};
use crate::commands::xr::helpers::types::{ManifestSource, ObserveArgs, ReclaimSpec};
...
pub(crate) fn run(args: &ObserveArgs) -> Result<(), Box<dyn Error>> {
    ...
    let mut live_notes = apply_live_aws(&spec, &mut manifest, &args.name, &args.aws_region)?;
    live_notes.extend(observe_workload_cluster_state(
        &spec,
        &mut manifest,
        &args.name,
        &args.aws_region,
...
fn observe_workload_cluster_state(
    spec: &ReclaimSpec,
    manifest: &mut Value,
    xr_name: &str,
    aws_region: &str,
    ...
    match spec.kind.as_str() {
        "AutoEKSCluster" => {
            observe_autoekscluster_workload_state(spec, manifest, xr_name, aws_region)
        }
        _ => Ok(Vec::new()),
    }
}

fn observe_autoekscluster_workload_state(
    spec: &ReclaimSpec,
    manifest: &mut Value,
    xr_name: &str,
    aws_region: &str,
