...
pub fn save_entry(state_dir: &Path, entry: &CatalogEntry) -> Result<(), Box<dyn Error>> {
    ...
    let path = dir.join(format!("{}.json", slug(&entry.name)));
...
fn discover_into_catalog(args: &DiscoverArgs) -> Result<(), Box<dyn Error>> {
    ...
    walk_gitops(&root, 0, 4, &mut found)?;
    if found.is_empty() {
        println!("No Environment documents found under {}", root.display());
        return Ok(());
    }
    for source in found {
        let name = environment_name_from_file(&source).unwrap_or_else(|| {
            source
                .parent()
...
fn set_enabled(
    ...
    let Some(index) = entries.iter().position(|entry| entry.name == name) else {
        return Err(format!("Environment {name:?} is not in the catalog; run `hops local env discover`").into());
    };
    ...
    if !enabled && !entry.enabled {
        println!("already disabled {}", entry.name);
        return Ok(());
    }
...
fn walk_gitops(
    ...
    let skip = matches!(
        ...
        Some(".git" | "node_modules" | "target" | ".worktrees" | ".kb")
    );
    if skip && depth > 0 {
        return Ok(());
    }
